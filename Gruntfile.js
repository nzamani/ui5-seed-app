...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		dir: {
			...
			testWebapp: "src/test/webapp",
			distWebapp: "dist/webapp",
			webAppResources : "src/main/resources/webAppResources"
		},

		// i.e. converts "nabi.seed.app" from ui5 app descriptor to "nabi/seed/app"
				...
				client: {
					openui5: {
						config: {
							...
							preload: "async",
							resourceRoots: {
								"nabi.seed.app": "./base/src/main/webapp",
								//DEPENDENCIES
								"nabi.seed.comp": "http://localhost:8080/test-resources/ui5-seed-comp/src/main/webapp",
								// NEVER DO THIS because it would lead to "non-instrumentalized" sources (i.e. coverage not detected):
								//"nabi.seed.app": "http://localhost:8080/..."
							}
						},
						tests: [
							"nabi/seed/app/test/unit/allTests",
							"nabi/seed/app/test/integration/AllJourneys"		// remove this in case it takes too long, i.e. tdd/dev time
						]
					}
				},
				reporters: ["progress"],
		...
		openui5_connect: {
			options: {
				...
				testresources : [
					"src/main/resources/webAppResources",
			...
			srcToDist: {
				files: [
					{	// first all resources (incl. lib and thirdparty) except content that shall never be deployed
						...
						cwd: "src/main/resources/sap-ui5-repo-upload/",
						dest: "./<%= dir.distWebapp %>/"
					}
				]
			},
	...
	grunt.registerTask("installSapui5", function() {
